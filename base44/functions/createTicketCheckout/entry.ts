...
Deno.serve(async (req) => {
  try {
    ...
    // Only allow redirect URLs on this app's own origin or its connected custom
    // domain to prevent open redirect.
    const appOrigin = new URL(req.url).origin;
    const ALLOWED_REDIRECT_ORIGINS = [appOrigin, "https://thedoorman.app"];
    const safeRedirect = (url, fallback) => {
      if (!url) return fallback;
      try {
        const u = new URL(url);
        return ALLOWED_REDIRECT_ORIGINS.includes(u.origin) ? url : fallback;
      } catch {
        return fallback;
      }
    };
    const successUrl = safeRedirect(success_url, `${ALLOWED_REDIRECT_ORIGINS[1]}/event/${tier.event_id}?payment=success`);
    const cancelUrl = safeRedirect(cancel_url, `${ALLOWED_REDIRECT_ORIGINS[1]}/event/${tier.event_id}?payment=cancelled`);
