-- ===========================================================================
-- Friend suggestions: mutuals → has-picture → random.
--
-- Keeps mutual-friend people first (by count), and within the same count now
-- ranks accounts WITH a profile picture above those WITHOUT, then applies the
-- stable per-user/per-day random order within each group (so pagination stays
-- consistent and it reshuffles daily).
--
-- Reproduces get_friend_suggestions from 20260907120000 verbatim, changing only
