-- ===========================================================================
-- Friend suggestions: mutuals first, then random.
--
-- Previously ordered by mutual-friend count desc, then full_name asc. Keep the
-- mutuals-first ordering (by count) but randomise everyone else instead of
-- listing them alphabetically. The randomness is a per-user, per-day
-- deterministic hash rather than random(): the Suggestions tab paginates, and
-- the RPC ranks the whole set each call — a volatile random() would reshuffle
-- between page fetches and cause repeats/skips. The hash is stable within a
-- day (so paging is consistent) and reshuffles daily.
--
-- Reproduces get_friend_suggestions from 20260906120000 verbatim, changing only
-- the window ORDER BY (keeps the Smoke-host + deleted-account filters).
-- ===========================================================================
...
begin
  ...
  from (
    ...
    select p.email, p.full_name, p.avatar_url, p.instagram,
           ...
           row_number() over (order by coalesce(m.n, 0) desc,
