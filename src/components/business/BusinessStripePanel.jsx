...
export default function BusinessStripePanel({ business }) {
  ...
  return (
    <div className="bg-card rounded-2xl border border-border p-4 mb-4">
      ...
      <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-mono mb-3">Separate from your personal account</p>

      {!connected ? (
        <div>
          <p className="text-xs text-muted-foreground mb-3 leading-relaxed">
            Connect a Stripe account for the business to receive ticket payouts directly to the business bank account.
          </p>
        ...
        <div className="space-y-3">
          <div className={`flex items-center gap-2 text-xs px-3 py-2 rounded-lg border ${active ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-400" : "border-amber-500/30 bg-amber-500/10 text-amber-400"}`}>
            ...
            <span className="flex-1">{active ? "Business Stripe account active — payouts enabled" : "Finish setup in Stripe to enable payouts"}</span>
          </div>
