...
// Pan/zoom editor so the user can reposition their profile picture inside the
// circle. Crops a square around the visible region and uploads via UploadFile.
export default function ProfilePictureEditor({ file, onSave, onClose }) {
  ...
  const V = 260; // viewport (circle) diameter in CSS px
  ...
  useEffect(() => {
    ...
    image.onload = () => {
      setImg(image);
      // start at cover scale * zoom 1, centered
  ...
  async function handleSave() {
    ...
    try {
      ...
      // image pixel coords of the viewport's top-left
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[60] flex items-center justify-center px-4">
      <div className="bg-card rounded-3xl border border-border w-full max-w-sm p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-heading font-bold text-lg">Adjust Photo</h2>
        ...
        <div className="flex justify-center mb-5">
          <div
            ref={viewportRef}
            className="rounded-full overflow-hidden bg-secondary touch-none cursor-grab active:cursor-grabbing relative"
